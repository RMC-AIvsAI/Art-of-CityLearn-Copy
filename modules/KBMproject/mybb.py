        ...
        **kwargs,
        ...
        :Keyword Arguments:
            * *starting_points* (``np.ndarray``)
                ...
                *kwarg starting_points has unexpected behaviour, use adv_init instead
        """
        starting_points = None #kwargs.get("starting_points") #has issues, while adv_init does the same this and works
        ...
        def check_starting_points(starting_points, y):
            starting_preds = np.argmax(self.estimator.predict(x=starting_points), axis=1)
            y_label = np.argmax(y, axis=1)
            if self.targeted:
                if not (starting_preds==y_label).all(): #starting points match target
                    logger.warning(f'Starting points had predicted class: {starting_preds}\nBut the target was: {y_label}\n')
                    return None #return the points that did work, and exclude those that didn't?
                else:
                    return starting_points
                    
            else:
                if not (starting_preds != y_label).all():#starting points are different calss from starting point
                    logger.warning(f'Starting points had predicted class: {starting_preds}\nwhich matched the prediction: {y_label}\n')
                    return None
                else:
                    return starting_points
                    

        #verify starting points, clear them if they're bad. unnessary if we don't use the starting points kwarg
        # if starting_points is not None:
        #     starting_points = check_starting_points(starting_points, y)
        #     logger.info('Starting point was not adversarial, searching for starting point')
        ...
        if starting_points is None:
            ...
            for i_x in range(x.shape[0]): #TODO vectorize or parallelize
                initial_sample = self._fast_init_sample( #original is _init_sample
                    x=x[i_x],
                    y=y_index[i_x],
                    y_p=preds[i_x],
                ...
                if initial_sample is None: #uses input sample x as starting points when no adv starting point is found
                    starting_points[i_x] = x[i_x] #why not just return this instead of None?
                else:
                    starting_points[i_x] = initial_sample[0]

        best_advs = starting_points

        #is the check even necessary? worst case the attack runs on initial samples.
        # if check_starting_points(starting_points, y) is None: # is this check preventing the attack for multiple samples when only some don't match?
        #     logger.warning('Failed to find advsersarial starting points, original samples returned')
        #     return x
        ...
        for step in range(1, self.steps + 1):
            ...
            exclude = classes #same as input y
...
#my changes below:
#should save points found as dict of class:sample, so avoid future searches
    def _fast_init_sample(
        self,
        x: np.ndarray,
        y: int,
        y_p: int,
        ...
        initial_sample = None
        init_batch = 10_000 #found tith trial and error, at 1000 and 100 the performance is similar to the default for loop
        #higher vlaues are slow when the x adv is easy to find

        #added below
        batches, init_remainder = divmod(self.init_size, init_batch)
        ...
        if self.targeted:
            ...
            if adv_init is not None and init_pred == y:
                logger.info("Adversarial initialization will be used for attack")
                return adv_init.astype(config.ART_NUMPY_DTYPE), init_pred

            # Attack unsatisfied yet and the initial image unsatisfied

            for _ in range(batches):
#tried generating rand_vec with pytroch, but the estimator only accepts cpu tensors so ther's nothing to be gained
                rand_vec = nprd.uniform(size=(init_batch,)+x.shape).astype(x.dtype) #generate vector of random samples
                rand_preds_vec = np.argmax(self.estimator.predict(rand_vec, batch_size=self.batch_size), axis=1) #axis is two because this is an array of samples
                target_samples = rand_vec[rand_preds_vec == y] #random samples of the correct class
                if target_samples.size != 0: #sample found
                    break

            if(init_remainder > 0): #if the number of init steps isn't divisible by batches
                rand_vec = nprd.uniform(size=(init_remainder,)+x.shape).astype(x.dtype) #generate vector of random samples
                rand_preds_vec = np.argmax(self.estimator.predict(rand_vec, batch_size=self.batch_size), axis=1) #axis is two because this is an array of samples
                target_samples = rand_vec[rand_preds_vec == y] #random samples of the correct class

            if target_samples.size == 0: #verify a sample of the target class was generated/not empty
                logger.warning("Failed to draw a random image that is adversarial, attack failed.")
            else:
                target_norms = np.linalg.norm(target_samples - rand_vec[0], ord=2) #using an observation as a placeholder for the initial sample
                closest_rand_sample = target_samples[np.argmin(target_norms)]# find the random obs closest to the initial sample
                # Binary search to reduce the l2 distance to the original image
                closest_rand_sample = self._binary_search(
                    current_sample=closest_rand_sample,
                    original_sample=x,
                    target=y,
                    norm=2,
                    clip_min=clip_min,
                ...
                initial_sample = closest_rand_sample, y

                logger.info("Found initial adversarial image for targeted attack.")
                
            # for _ in range(self.init_size): #TODO vectorize the loop, check all afterwards
            #     random_img = nprd.uniform(clip_min, clip_max, size=x.shape).astype(x.dtype) #size should be x.shape + init.size
...
#tried generating rand_vec with pytroch, but the estimator only accepts cpu tensors so ther's nothing to be gained
            rand_vec = np.random.uniform(size=(self.init_size,)+x.shape).astype(x.dtype) #generate vector of random samples
            rand_preds_vec = np.argmax(self.estimator.predict(rand_vec, batch_size=self.batch_size), axis=1) #axis is two because this is an array of samples
            target_samples = rand_vec[rand_preds_vec != y_p] #random samples of the correct class
            if target_samples.size == 0: #verify a sample of the target class was generated/not empty
                logger.warning("Failed to draw a random image that is adversarial, attack failed.")
            else:
                target_norms = np.linalg.norm(target_samples - rand_vec[0], ord=2) #using an observation as a placeholder for the initial sample
                ...
                initial_sample = closest_rand_sample, y_p
...
#my changes above, below fo reference
