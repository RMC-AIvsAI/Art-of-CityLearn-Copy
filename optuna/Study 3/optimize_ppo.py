...
def sample_ppo_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    n_steps = trial.suggest_int("exponent_n_steps", 4, 16) #from prev study, best value was the lowest of 8, with the next highest value being 16
...
def objective(trial: optuna.Trial) -> float:
    """CityLearn KPI objective
    ref for multi-objective: https://optuna.readthedocs.io/en/stable/tutorial/20_recipes/002_multi_objective.html"""
    ...
    try:
        ...
        costs = utils.format_kpis(eval_env)
        # Free memory.
        model.env.close()
        eval_env.close()
    ...
    kpis = 'electricity_consumption_total'
    score = costs[kpis]
    ...
    except Exception as e: #meant to catch ValueError: Record does not exist. When no best value is defined after the first trial, I'm unsure if other errors are possible
    ...
    return score #uses CityLeanr kpis/cost functions instead of rewards
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(sampler=sampler, 
                                pruner=pruner, 
                                direction="minimize", #list for multiple objectives 
                                storage=STORAGE, 
