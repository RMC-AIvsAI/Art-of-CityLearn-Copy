...
CLASSES = 11 #20 #try reducing? victim use 20 for [-1, 1], so we should use something like 11 for [0,1]
INPUTS = 31
DIR = os.getcwd()
FOLDS = 10
EPOCHS = 1000//FOLDS #500 for first
DATADIR = '20 bin PPO 500 results/'
NAME = f'{CLASSES} bin psychic for 20 bin PPO 500 Policy 0'
SOC_COL = 25
...
def load_S_binned_dataset(n_bins = 10):
    ...
    labels = pd.cut(next_soc,
                    bins=CLASSES, #np.linspace(next_soc.min.astype(int),
                                     #next_soc.max.astype(int),
                                     #n_bins+1), #assign bin boundaries
                    labels=False, #just return the bin number range(0, n_bins), 
                    include_lowest=True,)
    assert not df_obs.isnull().values.any(), 'labels contain nans'
    labels = labels.reset_index(drop=True)
    df_obs = df_obs.reset_index(drop=True)
    df_obs['label'] = labels
    assert not df_obs.isnull().values.any(), 'obs contains nans, reset indexes?'
    return df_obs
...
def define_model(trial):
    ...
    for i in range(n_layers):
        out_features = trial.suggest_int("n_units_l{}".format(i), 64, 512, step=16) #64-256 starting
...
class objective():
    ...
    def __call__(self,trial):
        ...
        #select batch size
        batch_size = 2**trial.suggest_int('bs_exp', 3, 10) #6-10 starting
...
if __name__ == "__main__":
    ...
    study.set_user_attr('epochs per split',EPOCHS)
    ...
    study.set_user_attr('torch manual seed',SEED)
    study.set_metric_names([f'mean accuracy of {FOLDS} splits'])
    
    
    try:
        study.optimize(objective(dataset=MyDataset(df=load_S_binned_dataset(n_bins=CLASSES))), 
                    n_trials=N_TRIALS, 
