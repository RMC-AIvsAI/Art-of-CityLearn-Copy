...
NAME = 'S Surrogate 20 bin PPO 500 Policy 0'
SOC_COL = 25
...
def load_S_dataset():
    ...
    assert not df_obs.isnull().values.any(), 'obs contains nans'

    #assign labels   
    labels = df_obs.iloc[1:,SOC_COL] #SOC for all but first sample
    assert not labels.isnull().values.any(), 'labels contains nans'
    df_obs = df_obs.iloc[:-1] #drop the last row since we have no corresponding label
    assert not df_obs.isnull().values.any(), 'obs contain nans'
    assert df_obs.shape[0] == labels.shape[0]
    labels = labels.reset_index(drop=True)
    df_obs = df_obs.reset_index(drop=True)
    df_obs['next_soc'] = labels
    assert not df_obs.isnull().values.any(), 'obs contains nans'
    return df_obs
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.data[idx], self.targets[idx].unsqueeze(dim=-1) #match pred dims
...
if __name__ == "__main__":
    ...
    try:
        study.optimize(objective(dataset=MyDataset(df=load_S_dataset())), 
                    n_trials=N_TRIALS, 
