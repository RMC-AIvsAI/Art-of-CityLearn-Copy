...
class CustomReward(RewardFunction):
    ...
    def calculate(self, observations: List[Mapping[str, Union[int, float]]]) -> List[float]:
        ...
        for o in observations:
            reward_list.append(o['net_electricity_consumption']) #reward is positive power consumption rather than the default negative power consumption
...
episodes = 2400
T = env.time_steps - 1
agent_name = f'default_adversary_PPO_{dataset_name}_{building}_{bins}_bins_default_rwd'
...
agent_name = f'default_adversary_PPO_{dataset_name}_{building}_{bins}_bins_default_rwd_{int(eval_callback.num_timesteps/T)}'

try:
    agent.save(f"Models/Adversary/{agent_name}")
except: #in case the dir is wrong somehow
    agent.save(f'{agent_name}')

print(utils.format_kpis(eval_env))